...
for (i in 1: length(vars_indices_keep)) {
  # By each subject, i, find the average of each activity by using mn84_90 <- with(datfrm, mean(score[year >= 1984 & year <= 1990]) ), basically, the 'with' funciton.
  for (j in 1: max(unique(Compiled_higherdata$Subject))) {
    for (k in 1:6){ 
    test[j+k-1,i] <- with(Compiled_higherdata, mean(Compiled_higherdata[,j][ActivityLabel == k & Subject == i]))
    # Need to think about how to organize this data. There will probably be a variable for each indication. That means multiple variables will then need to be bound together.
    }
  }
} # Also need to count the number of times activitylabel and subject is used here. 
